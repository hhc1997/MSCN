...
def evalrank(model_path, data_path=None, vocab_path=None, split="dev", fold5=False):
    ...
    with torch.no_grad():
        img_embs_A, cap_embs_A, cap_lens_A = encode_data(net_A, data_loader)
        img_embs_B, cap_embs_B, cap_lens_B = encode_data(net_B, data_loader)
    print(
        "Images: %d, Captions: %d"
    ...
    if not fold5:
        ...
        sims_A = shard_attn_scores(
            net_A, img_embs_A, cap_embs_A, cap_lens_A, meta_net_A, opt, shard_size=1000
        )
        sims_B = shard_attn_scores(
            net_B, img_embs_B, cap_embs_B, cap_lens_B, meta_net_B, opt, shard_size=1000
        )
        ...
        # caption retrieval
        (r1, r5, r10, medr, meanr) = i2t(img_embs_A.shape[0], sims, per_captions)
        print(
        ...
        # image retrieval
        (r1i, r5i, r10i, medri, meanr) = t2i(img_embs_A.shape[0], sims, per_captions)
        print(
